...
export default class Project extends React.Component{
  render(){
    ...
    const openIssueSection = (props => {
        ...
        return openIssues.map((issue) => (<div> <a href={issue.html_url} title={issue.body} alt={issue.body}> # {issue.id || ''}: {issue.title}</a></div>))
    ...
    return (<div className="sm-col-6 lg-col-4 border-box flex flex-stretch flex-wrap bg-dgray radius m2 ac-flex-start">
              <div className="col-12 border-bottom py1 px2">
                <a className="white" href={"https://github.com/"+project.name} target="_blank">{project.name}</a>
              </div>
              ...
              {contributors.map( (contributor) => {
                  if(contributor.contributions >= 100)
                      return <li className="p2" key={contributor.login}><a href={contributor.html_url}><img height="48px" width="48px" src={contributor.avatar_url} alt={contributor.login}/></a><p
                    className="contributionsGrand">{contributor.contributions}</p></li>
                  else if (contributor.contributions >= 10)
                      return <li className="p2" key={contributor.login}><a href={contributor.html_url}><img height="48px" width="48px" src={contributor.avatar_url} alt={contributor.login}/></a><p
                          className="contributions">{contributor.contributions}</p></li>
                  else
                      return <li className="p2" key={contributor.login}><a href={contributor.html_url}><img height="48px" width="48px" src={contributor.avatar_url} alt={contributor.login}/></a><p
                          className="contributionsPetit">{contributor.contributions}</p></li>
              })}
              </ul>
            {openIssueSection}
            </div>)
